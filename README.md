...
This repo contains my Proof-Of-Concept implementation of the brute-force approach to the Quadratic Assignment Problem. There are three programs:
...
The two solvers enumerate *every* permutation of the assignment vector and keep the minimum of `sum_ij FLG[i][j] * DST[AS[i]][AS[j]]`. They must agree on three things: the minimum cost, the iteration count - which is exactly `N!` for an `N x N` instance - and the minimizing assignment itself. See **Performance** below for the CPU/GPU comparison, and the file `cudaqap.md` for the full measurements.
...
%> make -f Makefile.cuda    # GPU version
```

Plain `make` and `make all` are equivalent and build both programs; `make clean` removes them.

`Makefile.cuda` defaults to `CUDA_VERSION = 12.9` and `CUDA_CC = 89` (Ada, `sm_89`). Change those two variables to match your CUDA installation and your GPU's Compute Capability if you want to compile for Blackwell (`sm_120`, `CUDA_VERSION = 13.2`).
...
`Makefile.clang` is not just the Linux Makefile with the compiler swapped. It
...
`_DARWIN_C_SOURCE` instead. It does not build `cudaqap`, since CUDA is no
longer available on MacOS. `clock_getres(3)` needs MacOS 10.12 or later.
...
```
%> ./qap -f ./sample-data/fldata-100.dat -d ./sample-data/dstdata-100.dat -p
Minimum cost: 165908
Iterations: 3628800
Flow Graph Vector:
...
```
%> ./cudaqap -s ./sample-data/chr12a.dat
Minimum cost: 9552
...
runtime projections beyond N=15.
...
The four largest runs actually carried to completion:
...
exactly `N!` times. For N=13 and N=14 the reported assignment was also
re-scored independently against the input matrices; for N=15 and N=16 only the
iteration count was checked, since printing the assignment means running the
search again.
